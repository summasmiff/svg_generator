defmodule SvgGenerator.Spirograph do
  ...
  """
  @big_R 37 # needs to be >= little_r
  @little_r 7
  @offset 15

  def x(rad) do
    (@big_R + @little_r) * :math.cos(rad) - (@little_r + @offset) * :math.cos( ( (@big_R+@little_r) /@little_r ) * rad )
  end

  def y(rad) do
    (@big_R + @little_r) * :math.sin(rad) - (@little_r + @offset) * :math.sin( ( (@big_R+@little_r) /@little_r ) * rad )
  end
